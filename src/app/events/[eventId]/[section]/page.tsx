...
import { WhoSaidHostPanel } from "@/components/who-said-host-panel";
import { DilettantesQuickEditor } from "@/components/dilettantes-quick-editor";
          ...
          <div className="event-kit-list">
            {items.length ? items.map((item, index) => (
              <article className={`event-kit-card ${item.do_not_use ? "event-kit-card--blocked" : ""}`} key={item.id}>
                ...
                {item.data.interactive_kind === "dilettantes" ? <DilettantesQuickEditor eventId={eventId} item={item} /> : <form action={updateEventKitItemAction.bind(null, eventId, item.id)} className="event-kit-card__body" noValidate>
                ...
                <WhoSaidHostPanel eventId={eventId} item={item} submissions={submissions} />
              </article>
