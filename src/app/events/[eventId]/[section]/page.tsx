    ...
    return (
      <AppShell>
        ...
        {!sessionMatches ? <section className="live-start"><span className="live-start__signal" /><h2>{session ? `Зараз активна сесія: ${session.mode}.` : requestedMode === "live" ? "Готові починати?" : "Перевіримо все без ризику."}</h2><p>{requestedMode === "live" ? "Після старту Public Screen отримає лише явно показані схвалені блоки." : "Репетиція використовує той самий sanitized public pipeline, але чітко позначена як тест."}</p><form action={startLiveSessionAction.bind(null, eventId, requestedMode)} noValidate><button className="button button--brand button--solid" type="submit">{session ? `Перемкнути на ${requestedMode}` : `Почати ${requestedMode}`}</button></form></section> : <section className="live-console">
          ...
          <div className="live-candidates"><LiveAutoplay eventId={eventId} candidates={candidates.map((item) => ({ id: item.id, title: item.title || "Без назви" }))} /><div className="section-heading"><div><span className="eyebrow">READY TO SHOW / {String(candidates.length).padStart(2, "0")}</span><h2>Схвалені блоки</h2></div></div>{candidates.length ? candidates.map((item, index) => <article className="live-item" key={item.id}><span className="live-item__index">{String(index + 1).padStart(2, "0")}</span><div><span className="eyebrow">{EVENT_KIT_TYPE_LABELS[item.item_type]}</span><h3>{item.title || "Без назви"}</h3><p>{item.content}</p></div><form action={showEventKitItemAction.bind(null, eventId, item.id)} noValidate><button className="button button--brand button--solid" type="submit">Показати →</button></form></article>) : <div className="status">Немає блоків зі статусом approved + public_allowed. Підготуйте їх у Event Kit.</div>}</div>
        </section>}
      </AppShell>
    );
  }
