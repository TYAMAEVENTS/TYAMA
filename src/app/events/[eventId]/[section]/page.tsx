...
import { advanceSlideshowAction, clearPublicScreenAction, endLiveSessionAction, revealInteractiveAction, showInteractiveIntroAction, showWelcomeQrAction, spinDilettantesWheelAction, startInteractiveAction, startLiveSessionAction } from "@/app/actions/live";
...
import { FamilyFeudHostPanel } from "@/components/family-feud-host-panel";
...
import { WelcomeQrSetup } from "@/components/welcome-qr-setup";
...
export default async function EventSectionPage({ params, searchParams }: { params: Promise<{ eventId: string; section: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  if (section === "responses") {
    ...
    for (const submission of submissions) for (const answer of submission.answers) {
      ...
      const prompt = answer.question?.prompt || "Питання";
  ...
  if (section === "event-kit") {
    const [items, submissions, query] = await Promise.all([listEventKitItems(eventId), listEventSubmissions(eventId), searchParams]);
    return (
      <AppShell>
        ...
        <section className="event-kit-layout">
            ...
            {items.length ? items.map((item, index) => (
              <article className={`event-kit-card ${item.do_not_use ? "event-kit-card--blocked" : ""}`} key={item.id}>
                ...
                <form action={updateEventKitItemAction.bind(null, eventId, item.id)} className="event-kit-card__body" noValidate>
                  ...
                  <div className="form-field"><label className="form-field__label" htmlFor={`kit-content-${item.id}`}>Матеріал / шпаргалка</label><textarea className="resize-none" id={`kit-content-${item.id}`} name="content" defaultValue={item.content ?? ""} /></div>
                ...
                <FamilyFeudHostPanel eventId={eventId} item={item} submissions={submissions} />
              </article>
          ...
          <aside className="create-questionnaire-panel event-kit-create">
            ...
            <hr />
            <span className="eyebrow">КЛУБ ДИЛЕТАНТІВ / ЧИСЛО</span><h2>Нове числове питання</h2>
            <form action={createDilettantesInteractiveAction.bind(null, eventId)} className="editor-form" noValidate>
              <div className="form-field"><label className="form-field__label" htmlFor="dilettantes-question">Питання про подію або героїв</label><textarea className="resize-none" id="dilettantes-question" name="question" placeholder="Скільки кілометрів разом проїхали молодята?" /></div>
              <div className="form-grid"><div className="form-field"><label className="form-field__label" htmlFor="dilettantes-answer">Правильне число</label><input id="dilettantes-answer" name="correctAnswer" inputMode="decimal" /></div><div className="form-field"><label className="form-field__label" htmlFor="dilettantes-unit">Одиниця</label><input id="dilettantes-unit" name="unit" placeholder="км, днів, чашок" /></div></div>
              <div className="form-field"><label className="form-field__label" htmlFor="dilettantes-consequence">Легке покарання найдальшій відповіді</label><input id="dilettantes-consequence" name="consequence" placeholder="Сказати тост із трьома випадковими словами" /></div>
              <button className="button button--brand button--solid" type="submit">Додати готову гру</button>
            </form>
            <hr />
            <span className="eyebrow">MANUAL / SAFE FALLBACK</span><h2>Новий блок</h2><form action={createEventKitItemAction.bind(null, eventId)} className="editor-form" noValidate><div className="form-field"><label className="form-field__label" htmlFor="kit-new-type">Тип</label><select id="kit-new-type" name="type"><option value="story">Історія</option><option value="fact">Факт</option><option value="question">Питання</option><option value="interactive">Інтерактив</option><option value="note">Нотатка</option><option value="warning">Важливо</option><option value="other">Інше</option></select></div><div className="form-field"><label className="form-field__label" htmlFor="kit-new-title">Назва</label><input id="kit-new-title" name="title" /></div><div className="form-field"><label className="form-field__label" htmlFor="kit-new-content">Матеріал</label><textarea className="resize-none" id="kit-new-content" name="content" /></div><button className="button button--brand button--solid" type="submit">Додати в Event Kit</button></form></aside>
        </section>
      </AppShell>
    );
  }
  if (section === "rehearsal" || section === "live") {
    ...
    const [items, session, liveState, submissions, questionnaires] = await Promise.all([listEventKitItems(eventId), getActiveLiveSession(eventId), getHostLiveState(eventId), listEventSubmissions(eventId), listQuestionnaires(eventId)]);
    const welcomeItem = items.find((item) => item.item_type === "media" && item.data.interactive_kind === "welcome_qr" && (item.status === "approved" || item.status === "used") && item.privacy_status === "public_allowed" && !item.do_not_use);
    const publishedGuestQuestionnaire = [...questionnaires].reverse().find((questionnaire) => questionnaire.status === "published" && (questionnaire.audience === "guest" || questionnaire.audience === "other"));
    const candidates = items.filter((item) => item !== welcomeItem && (item.status === "approved" || item.status === "used") && item.privacy_status === "public_allowed" && !item.do_not_use && (item.item_type === "interactive" || item.item_type === "media"));
    ...
    return (
      <AppShell>
        ...
        <div className="page-heading"><div><span className="eyebrow">{requestedMode === "live" ? "BATTLE MODE" : "SAFE PREVIEW"}</span><h1>{config.title}</h1><p>{config.description}</p></div>{sessionMatches ? <Link href={screenUrl} target="_blank" className="button button--neutral button--outline">Відкрити екран ↗</Link> : null}</div>
        {!sessionMatches ? <section className="live-start"><span className="live-start__signal" /><h2>{session ? `Зараз активна сесія: ${session.mode}.` : requestedMode === "live" ? "Готові починати?" : "Перевіримо все без ризику."}</h2><p>{requestedMode === "live" ? "Після старту Public Screen отримає лише явно показані схвалені блоки." : "Репетиція використовує той самий sanitized public pipeline, але чітко позначена як тест."}</p><form action={startLiveSessionAction.bind(null, eventId, requestedMode)} noValidate><button className="button button--brand button--solid" type="submit">{session ? `Перемкнути на ${requestedMode}` : `Почати ${requestedMode}`}</button></form></section> : <section className="live-console">
          <aside className="live-current"><span className="eyebrow">ЗАРАЗ НА ЕКРАНІ / REV {liveState?.revision ?? 0}</span><div className="live-current__preview"><span>{liveState?.public_payload.kind || liveState?.mode || "idle"}</span><h2>{liveState?.public_payload.title || "Екран очищено"}</h2><p>{liveState?.public_payload.content || "Public Screen тримає нейтральний brand state."}</p></div>{!welcomeItem ? <WelcomeQrSetup eventId={eventId} /> : null}<div className="live-current__controls live-current__controls--screen">{publishedGuestQuestionnaire && welcomeItem ? <form action={showWelcomeQrAction.bind(null, eventId, publishedGuestQuestionnaire.id, welcomeItem.id)} noValidate><button className="button button--brand button--solid" type="submit">Показати QR</button></form> : <p className="status">{publishedGuestQuestionnaire ? "Спочатку збережіть welcome-заставку." : "Спочатку опублікуйте guest-анкету."}</p>}<form action={clearPublicScreenAction.bind(null, eventId)} noValidate><button className="button button--neutral button--outline" type="submit">Прибрати з екрана</button></form><form action={endLiveSessionAction.bind(null, eventId)} noValidate><button className="button button--danger button--solid" type="submit">Завершити сесію</button></form></div><ShareTools url={screenUrl} label="Public Screen" showQr={false} /></aside>
          <div className="live-candidates"><LiveAutoplay eventId={eventId} candidates={slideshowCandidates.map((item) => ({ id: item.id, title: item.title || "Без назви" }))} /><div className="section-heading"><div><span className="eyebrow">READY TO SHOW / {String(candidates.length).padStart(2, "0")}</span><h2>Інтерактиви й слайдшоу</h2></div></div>{candidates.length ? candidates.map((item, index) => <article className="live-item" key={item.id}><span className="live-item__index">{String(index + 1).padStart(2, "0")}</span><div><span className="eyebrow">{item.item_type === "interactive" ? String(item.data.interactive_kind || "Інтерактив") : "Слайдшоу"}</span><h3>{item.title || "Без назви"}</h3><p>{item.content}</p></div><div className="live-item__actions"><form action={showInteractiveIntroAction.bind(null, eventId, item.id)} noValidate><button className="button button--neutral button--outline" type="submit">Заставка</button></form><form action={startInteractiveAction.bind(null, eventId, item.id)} noValidate><button className="button button--brand button--solid" type="submit">Почати</button></form>{item.item_type === "interactive" ? <><form action={revealInteractiveAction.bind(null, eventId, item.id)} noValidate><button className="button button--neutral button--outline" type="submit">Відкрити відповідь</button></form>{item.data.interactive_kind === "dilettantes" ? <form action={spinDilettantesWheelAction.bind(null, eventId, item.id)} noValidate><button className="button button--neutral button--outline" type="submit">Крутити колесо</button></form> : null}</> : <form action={advanceSlideshowAction.bind(null, eventId, item.id)} noValidate><button className="button button--neutral button--outline" type="submit">Наступний файл</button></form>}</div><FamilyFeudHostPanel eventId={eventId} item={item} submissions={submissions} liveControls /></article>) : <div className="status">Ще немає готових інтерактивів або слайдшоу. Зберіть їх із відповідей у два натискання.</div>}</div>
        </section>}
      </AppShell>
    );
  }
